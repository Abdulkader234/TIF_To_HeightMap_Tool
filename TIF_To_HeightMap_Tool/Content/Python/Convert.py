...
def convert_and_import(tif_path):
    ...
    result = {
        ...
        "not_square": False,          # True om bilden inte är kvadratisk (bredd != höjd)
    ...
    # Validering: Kvadratisk textur
    # Unreal kräver att heightmaps är kvadratiska (bredd == höjd)
    # En icke-kvadratisk heightmap resulterar i ett deformerat landskap

    if data.shape[1] != data.shape[0]:
        result["not_square"] = True
        result["error_message"] = (
            "Heightmap is not square: "
            + str(data.shape[1]) + "x" + str(data.shape[0])
